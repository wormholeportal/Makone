 ...
 * Play as an Asiatic black bear wandering an infinite procedural world.
...
export default async function createWorld(container) {
  ...
  // ─── Player Character (Asiatic black bear, anthropomorphic) ──────────
  ...
  function renderFrame(rawDt) {
    const dt = Math.min(rawDt || 0, 0.06)
    globalTime += dt
    if (paused) {
      composer.render()
      return
