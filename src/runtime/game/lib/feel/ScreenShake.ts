 ...
 *     render()
 *
 *   See: skills/games/03-architecture/update-loop-ordering.md
 *
 ...
 *
 * See: skills/games/01-feel/screen-shake-physics.md
 *
...
export type ShakeDecay = 'linear' | 'smooth' | 'instant'

export type ShakeConfig = {
  /** Peak displacement, meters (or radians for rotation). */
  amplitude: number
  /** Oscillation frequency in Hz (default 25). */
  frequency?: number
  /** Total duration in seconds. */
  duration: number
  /** How amplitude decays over duration (default 'smooth'). */
  decay?: ShakeDecay
  /** Translational axes to shake (default {x:1,y:1,z:0}). z is depth — usually skip. */
  axes?: { x?: number; y?: number; z?: number }
  /** Roll amplitude in radians (default 0). Tiny values feel best — 0.01–0.04. */
  roll?: number
}

type ActiveShake = ShakeConfig & {
  elapsed: number
  seedX: number
  seedY: number
  seedZ: number
...
export class ScreenShake {
  ...
  private continuous = new Map<number, ShakeConfig>()
  private nextId = 1
  ...
  private _expectedPos = new Vector3()
  private _expectedRoll = 0

  constructor(public camera: Camera) {}

  /** Add a one-shot shake. */
  add(config: ShakeConfig): void {
    this.active.push({
      ...config,
      elapsed: 0,
  ...
  addContinuous(config: ShakeConfig): number {
    const id = this.nextId++
    this.continuous.set(id, { ...config, duration: Infinity })
    return id
  }

  removeContinuous(id: number): void {
  ...
  private _apply(s: ActiveShake, progress: number): void {
  ...
  private _decay(kind: ShakeDecay, progress: number): number {
